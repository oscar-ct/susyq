...
const CustomButton = ({isDisabled = false, customClass="", children, onClick, type="button"}) => {
    ...
    return (
        <motion.button whileTap={{scale: 0.9}} onClick={onClick} className={`relative rounded-full ring-white ring-2 px-5 py-2.5 text-base ${customClass} ${isDisabled ? "bg-gray-100 text-gray-300" : "overflow-hidden group bg-cyan-600 relative md:hover:bg-gradient-to-r md:hover:from-lime-500 md:hover:to-lime-400 text-white md:hover:ring-2 md:hover:ring-offset-0 md:hover:ring-lime-400 md:transition-all md:ease-out md:duration-300"}`} disabled={isDisabled} type={type}>
