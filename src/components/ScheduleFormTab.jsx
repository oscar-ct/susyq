...
const ScheduleFormTab = ({ disabled, tabName, id, error }) => {
    ...
    return (
        <div className={"h-full w-4/12"} onClick={onClick}>
            <div className={`${disabled ? "bg-stone-200 text-gray-600 cursor-not-allowed" : "bg-cyan-600 text-white cursor-pointer"} ${activeTab === id && !disabled ? "opacity-100" : "opacity-60"} ${error ? "bg-red-500" : ""} ${services.length === 0 && id !== 0 ? "cursor-not-allowed" : ""} ${serviceSubmitted ? "!cursor-not-allowed" : ""} h-full rounded flex items-center justify-center sm:justify-start`}>
